...
namespace Inkcorperated
{
	class CollisionManager
	{
		...
		public void Colliding()
		{
			...
			// Checks collisions between the player and each drawn block on the screen
			...
			// Checks collisions between the player and each preset block on the screen
			foreach (Block block in controller.Level[0].MapBlocks)	//*****************CHANGE FOR DYNAMIC LEVEL*************
			{
				if (controller.LevelPlayer.Bounds.Intersects(block.Bounds))
				{
					Rectangle intersection = Rectangle.Intersect(controller.LevelPlayer.Bounds, block.Bounds);
